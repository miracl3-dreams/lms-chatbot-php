...
function __construct($paper='a4',$orientation='portrait'){
	...
	if (!is_array($paper)){
      ...
      case 'LEGAL': {$size = array(0,0,612.00,936.00); break;}
      case 'LST_CHECK': {$size = array(0,0,595,307.2); break;}
		}
...
function ezTable(&$data,$cols='',$title='',$options=''){
  ...
  if (!is_array($cols)){
    ...
    //20190527 commented by gian karlo due to deprecation in php 7.3
    // list($k,$v)=each($data);
    // if (!is_array($v)){
    //   return;
    // }

    //20190527 added by gian karlo as subsitute for the commented codes above to deprecation in php 7.3
    foreach ($data as $k => $v) 
    {
        if (!is_array($v))
        {
